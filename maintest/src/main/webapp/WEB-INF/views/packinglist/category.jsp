	...
	<c:forEach items="#{categoryList }" var="itemCategoryVO">
		<dd>		
			...
			<ul class="pd_item_list">
				<c:if test="${empty requestScope.listing and empty sessionScope.listing}"></c:if>
				<c:if test="${!empty requestScope.listing}">
					<c:forEach items="${listing }" var="listingVO">
						<c:if
							test="${listingVO.ca_num eq itemCategoryVO.ca_num }">
							<li class="row">
								<div>
									<input
									...
									<input type="text"  name="icount" class="icount" value="${listingVO.icount }"
										max="20" onfocus="getPreValue(this)" onchange="changeValue(this)"/>
									...
									<i class="fas fa-minus deleteItem"
										onClick="deletelist_f(this)"> </i>
								</div> <input type="hidden" name="icategory"
								value="${listingVO.ca_num }" /> <input type="hidden"
								name="inum" value="${listingVO.i_num }" /><input
								type="hidden" name="i_name" value="${listingVO.i_name }" /><input
