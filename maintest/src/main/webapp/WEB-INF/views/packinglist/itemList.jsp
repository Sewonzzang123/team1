				...
				<tbody>
					...
					<c:if test="${!empty requestScope.listing}">
						<c:forEach items="#{itemList }" var="itemVO">
							...
							<c:choose>
								<c:when test="${itemVO.i_num eq listingVO.itemVO.i_num and itemVO.itemCategoryVO.ca_num eq listingVO.CategoryVO.ca_num}">					
										<tr class="tab_box tab_box${itemVO.itemCategoryVO.ca_num }">
											<td><a href="#"> <i class="fas fa-minus"
													onClick="deletelist2_f(this)"></i> <i class="fas fa-plus"
													onClick="addlist_f(this)" style="display: none"></i></a></td>
											<td><div class="item${itemVO.i_num }" selected="false">${itemVO.i_name }</div></td>
										</tr>
									</c:when>	
									<c:otherwise>
