...
public class PagingSVCImpl implements PagingSVC {
	...
	@Override
	public RecordCriteria getRecCriteria(String btype, long catnum, int reqPage, long recNumPerPage, String searchType,
		...
		System.out.println(
				"ㅁ※※※※※※※※※※※※※※※※※※※※※※※※※※ recordCriteria.getTotalRec()     =   " + recordCriteria.getTotalRec());
...
//페이징 wrapper _   (레코드 / 페이징 / 검색어) 요소 wrapping 
	@Override
	public PagingComponent getPagingComponent(String btype, long catnum, int reqPage, long recNumPerPage,
			String searchType, String searchKeyword) {
		PagingComponent pagingComponent = new PagingComponent();

		// 각 criteria 필드멤버 변수 설정
		RecordCriteria recordCriteria = getRecCriteria(btype, catnum, reqPage, recNumPerPage, searchType,
				searchKeyword);
