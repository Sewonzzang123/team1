...
@Slf4j
public class UploadFilesUtils_old {
	...
	public static String fileUpload(String uploadPath, String fileName, byte[] fileData, String ymdPath) throws Exception {
		...
		// + 날짜 == > 저장경로
		//출력값 : D:\java\apache-tomcat-9.0.37\wtpwebapps\maintest\resources\file\2020\08\29
		...
		//a new File instance from a parent abstractpathname and a child pathname string. ) 
		...
		//D:\java\apache-tomcat-9.0.37\wtpwebapps\maintest\resources\file\2020\08\29\ + s 안에 저장
		...
		if(image.exists()) {
			...
			//출력값 : D:\java\apache-tomcat-9.0.37\wtpwebapps\maintest\resources\file\2020\08\29\s
			
			//출력값 : if and only if directory was created : true otherwise false
			
			  Thumbnails.of(image).size(THUMB_WIDTH, THUMB_HEIGHT).toFile(thumbnail); //Create a thumbnail and writes it to a File.
